source("/dssg/home/acct-clswt/clswt3/sim/ARTM_sim.R")
source("/dssg/home/acct-clswt/clswt3/sim/mbCARE.R")
source("/dssg/home/acct-clswt/clswt3/sim/ANCOMBC2.R")
library(doSNOW)
library(foreach)
library(ggplot2)
library("ggsci") 
...
FC <- 1
sim_total = n.sim
sim.seed = matrix(1:sim_total, n.sim, 1)
...
ARTM_sim <- function(n, K, L, beta_1k = 1, pi_DA, pi_trans, FC = 2, confounder = "FALSE", covariate = c("TRUE", "FALSE")){
  ...
  if(covariate == "TRUE"){
    ...
    alpha01_k <- matrix(0, nrow = K, ncol = 2)
    alpha01_k[,1] <- -1
    idx01 <- sample(1:K, 0.3 * K)
    alpha01_k[idx01,2] <- sample(c(runif(length(idx01), -2, -1),
                                   runif(length(idx01),  1,  2)),
                                 length(idx01))
    alpha10_k <- matrix(0, nrow = K, ncol = 2)
    alpha10_k[,1] <- 1
    idx10 <- sample(1:K, 0.3 * K) 
    alpha10_k[idx10,2] <- sample(c(runif(length(idx10), -2, -1),
                                   runif(length(idx10),  1,  2)),
                                 length(idx10))
  }else{
...
res <- foreach(i = 1:sim_total, .combine = 'cbind', .errorhandling = "pass") %dopar% {
  ...
  c(power_mbCARE_01, power_mbCARE_10, power_mbCARE_q, power_ANCOMBC_q, power_LinDA_q, power_ZIGMM_q, power_ZIBR_q,
    fdr_mbCARE_01, fdr_mbCARE_10, fdr_mbCARE_q, fdr_ANCOMBC_q, fdr_LinDA_q, fdr_ZIGMM_q, fdr_ZIBR_q)
}
stopCluster(cl)
...
res[is.na(res)]=0
res=res*100
rownames(res)=c("power.mbCARE.01", "power.mbCARE.10", "power.mbCARE.q", "power.ANCOMBC", "power.LinDA", "power.ZIGMM", "power.ZIBR",
                "fdr.mbCARE.01", "fdr.mbCARE.10", "fdr.mbCARE.q", "fdr.ANCOMBC", "fdr.LinDA", "fdr.ZIGMM", "fdr.ZIBR")


f = function(i){
  index = sim.seed[,i]
  data.frame(metric = rep(c("power", "FDP"), each = 2),
             state = rep(c("0-1", "1-0"), 2),
             value = rowMeans(res[c(1,2,8,9), index]))
}

power.fdr01 = foreach(i = 1:4, .combine=rbind) %do% f(i)
...
f1 = function(i){
  ...
  data.frame(metric = rep(c("power", "FDP"), each=5),
             ...
             value = rowMeans(res[c(3:7,10:14), index]))
}
