...
res <- foreach(i = 1:sim_total, .combine = 'cbind', .errorhandling = "pass") %dopar% {
  ...
  if (inherits(out_mbCARE, "try-error")) {
    ...
    power_mbCARE_01 <- length(intersect(id_mbCARE_01,id0))/length(id0)
    fdr_mbCARE_01 <- 1-length(intersect(id_mbCARE_01,id0))/length(id_mbCARE_01)
    power_mbCARE_10 <- length(intersect(id_mbCARE_10,id0))/length(id0)
    fdr_mbCARE_10 <- 1-length(intersect(id_mbCARE_10,id0))/length(id_mbCARE_10)
  }
  ...
  if (inherits(f, "try-error")) {
    ...
    power_ZIGMM_0 <- length(intersect(id_ZIGMM_q,id0))/length(id0)
    fdr_ZIGMM_0 <- 1-length(intersect(id_ZIGMM_q,id0))/length(id_ZIGMM_q)
  }
  ...
  if (inherits(out_ZIBR, "try-error")) {
    ...
    power_ZIBR_0 <- length(intersect(id_ZIBR_q,id0))/length(id0)
    fdr_ZIBR_0 <- 1-length(intersect(id_ZIBR_q,id0))/length(id_ZIBR_q)
  }
  
  c(power_mbCARE_01, power_mbCARE_10, power_ZIGMM_0, power_ZIBR_0,
    fdr_mbCARE_01, fdr_mbCARE_10, fdr_ZIGMM_0, fdr_ZIBR_0)
}
stopCluster(cl)
...
res[is.na(res)]=0
res=res*100
rownames(res)=c("power.mbCARE.01", "power.mbCARE.10", "power.ZIGMM.0", "power.ZIBR.0",
                "fdr.mbCARE.01", "fdr.mbCARE.10", "fdr.ZIGMM.0", "fdr.ZIBR.0")


##########line plot#########
f = function(i){
  index = sim.seed[,i]
  data.frame(class = rep(c("power", "FDP"), each = 4),
             method = rep(c("mbCARE01", "mbCARE10", "ZIGMM", "ZIBR"), 2),
             value = rowMeans(res[, index]))
}

power.fdr = foreach(i = 1:4, .combine=rbind) %do% f(i)

out = data.frame(L = rep(c("5", "10", "20", "50"), each = 8),
                 power.fdr)
colnames(out) = c("L", "class", "method", "value")
out$L = factor(out$L, levels = c("5", "10", "20", "50"))
out$class = factor(out$class, levels =c("power", "FDP"))
out$method = factor(out$method, levels =c("mbCARE01", "mbCARE10", "ZIGMM", "ZIBR"))
...
p = p + geom_line(aes(group = method, col = method)) +
  ...
  scale_color_manual(values = c("mbCARE01" = "#E64B35FF", "mbCARE10" = "#4DBBD5FF", 
                                "ZIGMM" = "#3C5488FF", "ZIBR" = "#F39B7FFF")) + 
  scale_fill_manual(values = c("mbCARE01" = "#E64B35FF", "mbCARE10" = "#4DBBD5FF", 
                               "ZIGMM" = "#3C5488FF", "ZIBR" = "#F39B7FFF"))
p
