...
ncores <- 64
n.sim <- 100
K <- c(30, 50, 100, 200) # Number of subjects
n <- 50  # Number of taxons
...
DATA <- foreach(i = 1:4, .combine='c') %do% {
  ...
  foreach(j = sim.seed[,i], .packages = c("MASS", "dplyr"), .combine='c') %dopar% {
    set.seed(j)
    data <- ARTM_sim(n = n, K = K.i, L = L, pi_DA = pi_DA, 
...
out = data.frame(K = rep(c("30", "50", "100", "200"), each = 10),
                 power.fdr)
colnames(out) = c("K", "class", "method", "value")
out$K = factor(out$K, levels =c("30", "50", "100", "200"))
...
p1 = ggplot(out, aes(x = K, y = value, col = method)) + theme_bw()
p1 = p1+ geom_line(aes(group = method, col = method)) +
  ...
  labs( y = 'FDP and power (%)', x =  expression(italic(K)))+ 
