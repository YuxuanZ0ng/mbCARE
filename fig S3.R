...
DATA <- foreach(i = 1:4, .combine='c') %do% {
  ...
  foreach(j = sim.seed[,i], .packages = c("MASS", "dplyr"), .combine='c') %dopar% {
    set.seed(j)
    data <- ARTM_sim(n = n, K = K, L = L, pi_DA = pi_DA, 
                     pi_trans = pi_trans, FC = FC.i, covariate = covariate)
    list(data)}
}
stopCluster(cl)
...
out = data.frame(FC = rep(c("0.5", "1", "2", "3"), each = 10),
                 power.fdr)
colnames(out) = c("FC", "class", "method", "value")
out$FC = factor(out$FC, levels =c("0.5", "1", "2", "3"))
...
p1 = ggplot(out, aes(x = FC, y = value, col = method)) + theme_bw()
p1 = p1+ geom_line(aes(group = method, col = method)) +
  ...
  labs( y = 'FDP and power (%)', x = expression(Delta))+ 
