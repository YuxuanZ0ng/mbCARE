...
DATA <- foreach(i = 1:4, .combine='c') %do% {
  ...
  foreach(j = sim.seed[,i], .packages = c("MASS", "dplyr"), .combine='c') %dopar% {
    set.seed(j)
    data <- ARTM_sim(n = n, K = K, L = L, beta_1k = beta_1.i, pi_DA = pi_DA, 
...
out = data.frame(beta1 = rep(c("0.5", "1", "1.5", "2"), each = 10),
                 power.fdr)
colnames(out) = c("beta1", "class", "method", "value")
out$beta1 = factor(out$beta1, levels =c("0.5", "1", "1.5", "2"))
...
p1 = ggplot(out, aes(x = beta1, y = value, col = method)) + theme_bw()
p1 = p1+ geom_line(aes(group = method, col = method)) +
  ...
  labs( y = 'FDP and power (%)', x = expression(eta))+ 
